	...
	var CrmPowerPane = {
		...
		RegisterEvents: function () {
			...
			$("#pap_logicalnames").click(function () {
				try {
					...
					Xrm.Page.ui.controls.forEach(function (c) {
						...
						const controlNode =
							document.querySelector(`label[id$="${controlName}-field-label"]`) ||
							contentPanels[0].contentWindow.document.getElementById(controlName);

						if (!controlNode) { return; }

						if (!c.getAttribute) {
							createSchemaNameInput(controlName, window.top.document.getElementById(`${controlName}_d`));
						} else {
							if (!c.getVisible()) {
								return;
							}
					...
					Xrm.Page.ui.tabs.forEach((t) => {
						...
						t.sections.forEach((s) => {
							...
							if (s.getVisible()) {
								createSchemaNameInput(
									sectionName,
									window.top.document.querySelector(`table[name="${sectionName}"]`) ||
									window.top.document.querySelector(`section[data-id$="${sectionName}"]`)
									, "red"
								);
							}
						});
			...
			$("#pap_clearlogicalnames").click(function () {
				...
				contentPanels[0].contentWindow.document.querySelectorAll('.levelupschema').forEach((x) => x.remove());
